...
other_embedding_path=dataset_path+'{}/input/CBOW_node_embedding_144.pkl'.format(dataset_name)

save_model_dir='E:/save/Causalmodel/{}/'.format(dataset_name)+task+'/otheremb'
...
def _train_b():
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        with tf.compat.v1.variable_scope("Model"):
            ...
            for step in range(start_global_step,50):
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        with tf.compat.v1.variable_scope("Model"):
            ...
            for i in range(len(real_lenth_encounter_list)):
                ...
                loss_all += loss
                prediction_list.append(prediction)
            pre, rec, f1 ,acc= pre_rec_f1(result_input_list, prediction_list)
            print("valid loss: {},acc: {},pre: {},rec: {},f1: {}".format(loss_all / count,acc/count, pre / count, rec / count, f1 / count))
            _test_b()
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        with tf.compat.v1.variable_scope("Model"):
            ...
            for j in range(1):
                ...
                f = "end"+str(embedding_size)+".txt"
                with open(f, "a") as file:
                    file.write("auc: {}, acc: {},pre: {},rec: {},f1: {}".format(auc,acc, pre, rec,f1) + '\n')

def _train_m():
    start_global_step = 1
    hps,_,input_list=get_input_batch(FLAGS.mode,embedding_path,dict_list_path,input_path_train)
    f= open(other_embedding_path,'rb')
    ...
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        with tf.compat.v1.variable_scope("Model"):
            ...
            is_edreg_input_list = input_list[8]
            result_input_list=input_list[9]
            for step in range(start_global_step,26):
                count=0
                loss_all=0
                pre5_all=0
                rec5_all=0
...
def _valid_m():
    hps, _, input_list = get_input_batch('train', embedding_path,dict_list_path, input_path_valid)
    f= open(other_embedding_path,'rb')
    embedding_list=pickle.load(f)
    ckpt = tf.train.get_checkpoint_state(save_model_dir)
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
...
def _test_m():
    hps, _, input_list = get_input_batch('test', embedding_path,dict_list_path, input_path_test)
    f= open(other_embedding_path,'rb')
    embedding_list=pickle.load(f)
    ckpt = tf.train.get_checkpoint_state(save_model_dir)
    with tf.Graph().as_default(), tf.compat.v1.Session() as sess:
        with tf.compat.v1.variable_scope("Model"):
            ...
            print("test,pre_5: {},rec_5: {},pre_10: {},rec_10: {},pre_15: {},rec_15: {}"
                  .format(pre5_all / count, rec5_all / count, pre10_all / count, rec10_all / count,
                          pre15_all / count, rec15_all / count))
            f = "CBOW_diag" + str(embedding_size) + ".txt"
            with open(f, "a") as file:
                file.write("pre_5: {},rec_5: {},pre_10: {},rec_10: {},pre_15: {},rec_15: {}"
                  .format(pre5_all / count, rec5_all / count, pre10_all / count, rec10_all / count,
                          pre15_all / count, rec15_all / count) + '\n')
